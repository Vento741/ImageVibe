...
export function ParamsPanel() {
  ...
  useEffect(() => {
    const load = () => {
      ipc.invoke('catalog:list').then((loaded: Group[]) => {
        setGroups(loaded);
        if (!useGenerateStore.getState().selectedModelId) {
          const first = loaded.find((group) => group.category === selectedCategory)?.models[0]
            ?? loaded.flatMap((group) => group.models)[0];
          if (first) setSelectedModelId(first.id);
        }
      }).catch(() => {});
    };
    load();
    return ipc.on('catalog:prices-updated', load);
  }, []);
