...
export interface KieModel {
  ...
  /**
   * Ключи схемы, без которых сервис отвергает запрос (замер 3a). Роли сюда не входят.
   *
   * Такой параметр предвыбирается в контроле значением по умолчанию: отсутствовать он
   * не может, а тихая подстановка в момент отправки скрыла бы от пользователя то, что
   * реально уходит в оплачиваемый запрос.
   */
  required: string[];
  roles: ModelRoles;
  docUrl: string;
